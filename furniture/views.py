...
def furniture_detail(request: HttpRequest, furniture_slug: str) -> HttpResponse:
    ...
    if furniture.selected_fabric_brand:
        fabric_categories = FabricCategory.objects.filter(brand=furniture.selected_fabric_brand)
    return render(
        request,
        "furniture/furniture_detail.html",
        {"furniture": furniture, "parameters": parameters, "fabric_categories": fabric_categories},
    )
