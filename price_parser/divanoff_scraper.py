...
FABRIC_CATEGORY_LABELS = [
    "Стандарт", "Категорія I", "Категорія II", "Категорія III",
...
def _apply_price_formula(raw_price: Decimal) -> Decimal:
    """Base price = raw catalog price × multiplier + addon (DivanoffPriceConfig).

    Fabric-category step (fabric_value) стосується лише різниці між
    категоріями тканини і рахується окремо з сирих цін — формула сюди
    не застосовується.
    """
    from price_parser.models import DivanoffPriceConfig

    config = DivanoffPriceConfig.get()
    return round(raw_price * config.price_multiplier + config.price_addon, 0)
...
def _ensure_divanoff_brand():
    ...
    # Migrate old "Категорія 0" → "Стандарт"
    FabricCategory.objects.filter(brand=brand, name="Категорія 0").update(
        name="Стандарт", price=Decimal("0")
    )
...
class DivanoffScraper:
    ...
    def run_import(
        ...
        for idx, url in enumerate(all_urls, 1):
            ...
            for pr in matched:
                ...
                price_0 = _apply_price_formula(pr.all_prices[0] or pr.price)
    ...
    def _update_existing(self, furniture, pr: "PriceRow", dry_run: bool) -> bool:
        new_price = _apply_price_formula(pr.all_prices[0] or pr.price)
        ...
        for idx, url in enumerate(all_urls, 1):
            ...
            for pr in matched:
                ...
                if update_fields:
                    ...
                    self._log(f"  {article_code}: {new_price} грн")
                    stats["updated"] += 1
