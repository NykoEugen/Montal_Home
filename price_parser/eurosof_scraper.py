...
EXTRA_SUBCATEGORIES = {
    PRODUCT_TYPE_KANAPE: {
        "name": "Крісла розкладні (Eurosof)",
        "slug": "krisla-rozkladni-eurosof",
    },
    PRODUCT_TYPE_MINI: {
        "name": "Міні-дивани (Eurosof)",
...
class EurosofImporter:
    ...
    def run(
        ...
        # Step 1: parse catalog (straight sofas or corner sofas)
        self._log("Читаємо Excel-каталог...")
        parser = EurosofCatalogParser(self.xlsx_path)
        catalog_products = parser.parse_corner() if corner else parser.parse()
    ...
    def _create_product(
        ...
        display_name = f"{sp.h1_name}{mod_label}"
