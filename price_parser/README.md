...
- **Веб-парсер постачальника**: Пошук товарів на сайті постачальника через `robots/sitemap` + сторінку пошуку
...
### 7. XML/YML фід постачальника

Тип конфігурації: **Supplier Feed Config** (адмінка `price_parser/supplierfeedconfig/`).

Призначений для фідів у форматі YML (Yandex Market Language) або аналогічних XML.

**Налаштування матчингу артикулу:**

| Поле | Default | Опис |
|------|---------|------|
| `article_tag_name` | `model` | XML-тег з артикулом. Для Vetro — `article`, для більшості — `model` |
| `article_prefix_parts` | `0` | Скільки частин через `-` брати як базовий код. `0` = повний артикул |

**Приклад (Vetro):**
- Фід містить: `<article>S-120-cappuccino-velvet</article>`
- В БД: `article_code = "S-120"`
- Налаштування: `article_tag_name=article`, `article_prefix_parts=2`
- Результат: `S-120-cappuccino-velvet` → `S-120` → матч знайдено

Якщо один товар має кілька офферів (різні кольори з однаковою ціною) — оновлюється ціна від першого матчу, решта пропускаються через `seen_furniture`.

Запуск:
```bash
# Тест без оновлення БД
python manage.py update_prices --supplier-feed-id 1 --test

# Оновлення через адмінку: кнопка "Оновити ціни" в SupplierFeedConfig
```

### 8. Веб-парсер цін постачальника

Новий тип конфігурації: **Supplier Web Config** (адмінка `price_parser/supplierwebconfig/`).

Що робить:
- бере базовий URL постачальника;
- дозволяє обмежити оновлення лише вибраними категоріями товарів;
- дозволяє задати CSS-селектор блоку, де знаходиться ціна (для універсальності під різні шаблони сайтів);
- читає `robots.txt`, знаходить `Sitemap`;
- збирає URL товарів із `sitemap.xml`;
- шукає товар спочатку за артикулом, потім за назвою;
- парсить ціну з блоку:
  - `<del>` = базова ціна
  - `<ins>` = акційна
  - якщо `<del>` відсутній, `<ins>` вважається базовою ціною;
- оновлює `Furniture.price` / `Furniture.promotional_price` / `Furniture.is_promotional`.

Запуск з консолі (ручний режим, наприклад раз на тиждень):

```bash
# Тест без оновлення БД
python manage.py update_supplier_web_prices --config-id 1 --test

# Оновлення для конкретної конфігурації
python manage.py update_supplier_web_prices --config-id 1

# Оновлення всіх активних веб-конфігурацій
python manage.py update_supplier_web_prices --all
```
