...
class Command(BaseCommand):
    help = "Створює підкатегорії для розділу 'Матраци'."

    def handle(self, *args, **options):
        category = Category.objects.filter(name__iexact="Матраци").first()
        if not category:
            raise CommandError("Категорію 'Матраци' не знайдено.")

        created = 0
        updated = 0

        for name in MATTRESS_SUBCATEGORIES:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Готово. Створено: {created}, оновлено: {updated}, всього: {len(MATTRESS_SUBCATEGORIES)}"
            )
        )
